...
실제로 집에서 현재 반려견을 키우고 있기도 하고, 프로젝트 주제를 선정을 하는 과정에서 나의 실제 일상생활 상황에 대입해서 개발을 하면 재미와 흥미를 갖고 할 수 있을 것 같아 반려견 돌봄 서비스로 주제를 선정하게 되었습니다.  
...
<ol>소셜 로그인: Google OAuth 2.0 지원</ol>

<ol>JWT 기반 인증: Access & Refresh Token을 활용한 보안 관리</ol>  
...
(돌봄 예약 가능 날짜를 등록한 펫시터 중 현재 예약이 가능한 펫시터만 조회(+DTO로 직접 조회, 페이징))<br><br><br>
...
(선택한 돌봄사의 정보와 스펙들을 보여주고, 해당 돌봄사에게 작성된 모든 돌봄 리뷰를 조회(+리뷰는 최상위 5개 조회 후, 이후 비동기로 5개 씩 페이징))<br><br><br>
...
또, 페이징도 서버에서 페이지 항목 수를 정해서 클라이언트에게 응답을 하는 방식만이 있는 줄 알았지만, 대규모 큰 프로젝트 경우에는 프론트에서 디바이스 크기에 맞춰 페이징 크기를 정한다는 것도 알게 되었고, 도메인 주도 개발(DDD)에 대해서도
익히고 신경쓰게 되는 경험을 해보았습니다.
